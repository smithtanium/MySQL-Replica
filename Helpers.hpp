...
namespace ECE141 {
  ...
  //commented out the old in_array
  bool in_array(T (&anArray)[aSize], const T &aValue) {
...
//    }
	...
	static DataTypes getDatatype(Keywords aKeyword) {
		switch(aKeyword) {
			case Keywords::char_kw:
			case Keywords::varchar_kw:
    ...
    static std::string to_string(const Value &input) {
    ...
    static bool checkDouble(std::string &aString) {
        try {
            float aDouble = std::stod(aString);
            return true;
        }
        catch (...) {
            return false;
    ...
    static bool checkInt(std::string &aString) {
        try {
            int aInt = std::stoi(aString);
            return true;
        }
        catch (...) {
            return false;
    ...
    static bool checkUINT(std::string &aString) {
        try {
            uint32_t aUINT = std::stoi(aString);
            return true;
        }
        catch (...) {
            return false;
